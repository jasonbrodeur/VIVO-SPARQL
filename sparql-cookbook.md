...
# Example 8. Bulk insert an assertion (e.g. rdf:type <http://vivoweb.org/ontology/core#FacultyMember>) to many individuals

1. Use CONSTRUCT in SPARQL query to create a bulk upload file 
```sparql
PREFIX rdf:      <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX rdfs:     <http://www.w3.org/2000/01/rdf-schema#>
PREFIX xsd:      <http://www.w3.org/2001/XMLSchema#>
...
PREFIX foaf:     <http://xmlns.com/foaf/0.1/>
PREFIX geo:      <http://aims.fao.org/aos/geopolitical.owl#>
PREFIX mcmaster: <https://experts.mcmaster.ca/ontology/mcmaster#>
PREFIX obo:      <http://purl.obolibrary.org/obo/>
PREFIX ocrer:    <http://purl.org/net/OCRe/research.owl#>
PREFIX ocresd:   <http://purl.org/net/OCRe/study_design.owl#>
PREFIX skos:     <http://www.w3.org/2004/02/skos/core#>
...
CONSTRUCT { ?x a vivo:FacultyMember .}
WHERE {?x a mcmaster:Internal .
    FILTER (!regex(str(?x), "institutional")) .}

```
Save the resulting file as N3 (just a preference)  

2. Use the **Add/Remove RDF data** tool to bulk upload data
- ensure that *add instance data (supports large data files)* is selected
- Select the proper format (e.g. N3)
- Click submit
